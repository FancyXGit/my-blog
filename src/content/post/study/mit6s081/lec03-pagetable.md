...
:::tip
事实上MMU输出的物理地址还要经过CPU内部的内存控制器(MC)，控制器会对地址做区间判断  

- 如果地址落在 DRAM区间（比如 > 0x80000000），控制器会把地址转换成行/列地址信号，通过DDR物理通道发给内存条
- 如果地址落在 MMIO区间（比如 < 0x80000000），控制器就不会把请求发往内存通道，而是将其重定向到连接外设的总线（如PCIe总线）
